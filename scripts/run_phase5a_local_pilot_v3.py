"""Run the classification-first Phase 5A-L local Ollama judge experiment.

V3 keeps the frozen semantic groundedness task and 120-ID manifest, while
making the primary output contract only ``label`` and ``confidence``. Evidence
explanations are intentionally outside the primary prediction path.
"""
...
PILOT_ID = "ragtruth-local-llm-judge-pilot-v3"
...
DEFAULT_STATE = REPO_ROOT / "reports" / "phase5a-local-pilot-v3-state.json"
DEFAULT_REPORT = REPO_ROOT / "reports" / "phase5a-local-pilot-v3.json"
DEFAULT_PREFLIGHT = REPO_ROOT / "reports" / "phase5a-local-v3-preflight.json"
DEFAULT_DIAGNOSTICS = REPO_ROOT / "reports" / "phase5a-local-v3-failure-diagnostics.json"
V1_STATE = REPO_ROOT / "reports" / "phase5a-local-pilot-state.json"
V2_STATE = REPO_ROOT / "reports" / "phase5a-local-pilot-v2-state.json"
V1_REPORT = REPO_ROOT / "reports" / "phase5a-local-pilot-v1.json"
V2_REPORT = REPO_ROOT / "reports" / "phase5a-local-pilot-v2.json"
...
SYNTHETIC_PREFLIGHT_CASES = (
    ("v3-grounded-1", "The source says the value is seven.", "The value is seven.", "GROUNDED"),
    ("v3-grounded-2", "The store opens at nine.", "The store opens at nine.", "GROUNDED"),
    (
        "v3-hallucinated-1",
    ...
    ("v3-repeat-long", "A verified fact is seven. " * 160, "A verified fact is seven.", "GROUNDED"),
)
...
def _preflight(args: argparse.Namespace) -> dict[str, object]:
    ...
    total = len(SYNTHETIC_PREFLIGHT_CASES)
...
def _load_passing_preflight(path: Path) -> dict[str, Any]:
    if not path.is_file():
        raise RuntimeError("V3_PREFLIGHT_NOT_FOUND")
    payload = json.loads(path.read_text(encoding="utf-8"))
    config = _transport_config()
    if not isinstance(payload, dict) or payload.get("LOCAL_PREFLIGHT") != "PASS":
        raise RuntimeError("V3_PREFLIGHT_NOT_PASSED")
    if (
        ...
        or payload.get("model_digest_match_count") != len(SYNTHETIC_PREFLIGHT_CASES)
        or payload.get("primary_schema_version") != CLASSIFICATION_SCHEMA_VERSION
        or payload.get("transport_prompt_hash") != CLASSIFICATION_TRANSPORT_PROMPT_SHA256
    ):
        raise RuntimeError("V3_PREFLIGHT_FROZEN_CONFIG_MISMATCH")
    if (
        payload.get("semantic_prompt_hash") != JUDGE_PROMPT_SHA256
        or payload.get("schema_hash") != config.schema_hash
    ):
        raise RuntimeError("V3_PREFLIGHT_SEMANTIC_HASH_MISMATCH")
    return payload
...
def _record_predictions(records: Sequence[PilotRunRecord]):
    return {
...
def _classification_validator_rule(content: str) -> str | None:
    try:
    ...
    try:
        JudgeClassificationDecision.model_validate(parsed)
        ...
        for item in error.errors():
            ...
            if error_type == "extra_forbidden":
                ...
                rules.append("INVALID_LABEL_ENUM")
                ...
                rules.append(f"{error_type.upper()}:{location}")
        return ",".join(sorted(set(rules))) or "SCHEMA_VALIDATION"
    return None


class _FailureDiagnostics:
    """Atomically retain failed V3 outputs for offline technical attribution."""
    ...
    def __init__(self, path: Path) -> None:
        ...
        if path.is_file():
            ...
            if not isinstance(payload, dict) or payload.get("experiment_id") != PILOT_ID:
                raise RuntimeError("V3_DIAGNOSTIC_EXPERIMENT_MIX")
            existing = payload.get("attempts", [])
            if not isinstance(existing, list):
                raise RuntimeError("V3_DIAGNOSTIC_ARTIFACT_INVALID")
    ...
    def append(self, trace: JudgeEvaluationTrace, content: str | None, *, attempt: int) -> None:
        ...
        item: dict[str, object] = {
            ...
            "json_parse_status": (
                "EMPTY_CONTENT"
                if trace.api_success and not content
                else "PASS"
                if content and _classification_validator_rule(content) != "INVALID_JSON"
                else "FAIL"
            ),
            "schema_validation_status": (
                "PASS" if content and _classification_validator_rule(content) is None else "FAIL"
            ),
            "validator_rule": _classification_validator_rule(content)
            if content
            else "EMPTY_CONTENT",
        ...
        _write_json(
            self.path,
            {
                "schema_version": "phase5a-local-v3-failure-diagnostics-v1",
                "experiment_id": PILOT_ID,
                "transport_version": CLASSIFICATION_TRANSPORT_VERSION,
...
def _assert_isolated_paths(state: Path, report: Path) -> None:
    forbidden = (V1_STATE, V2_STATE, V1_REPORT, V2_REPORT)
    if state.resolve() in {path.resolve() for path in forbidden} or report.resolve() in {
        path.resolve() for path in forbidden
    }:
        raise RuntimeError("V3_STATE_REPORT_MUST_NOT_OVERWRITE_PRIOR_EXPERIMENT")


def _read_prior_status(path: Path, default_count: int, default_status: str) -> dict[str, object]:
    if not path.is_file():
        return {"successful_at_end": default_count, "status": default_status}
    payload = json.loads(path.read_text(encoding="utf-8"))
    return {
        "successful_at_end": payload.get("successful_at_end", default_count),
...
def _run_pilot(args: argparse.Namespace) -> dict[str, object]:
    ...
    if manifest.pilot_id != FROZEN_MANIFEST_ID or manifest.sampling_seed != FROZEN_SEED:
        raise RuntimeError("V3_MANIFEST_MISMATCH")
    ...
    _assert_isolated_paths(args.state, args.report)
    ...
    def observe(record: PilotRunRecord) -> None:
        ...
        progress_count += 1
        print(
            f"V3_PROGRESS success={progress_count}/120 example_id={record.example_id}", flush=True
        )
    ...
    hhem_summary = _baseline_summary(
        "hhem-2.1-open", baseline["hhem"], ground_truth, example_metadata
    )
    heuristic_summary = _baseline_summary(
        "heuristic-baseline", baseline["heuristic"], ground_truth, example_metadata
    )
    v1_status = _read_prior_status(V1_REPORT, 80, "INCOMPLETE_LOCAL_TECHNICAL_FAILURE")
    v2_status = _read_prior_status(V2_REPORT, 84, "INCOMPLETE_LOCAL_TECHNICAL_FAILURE")
    gemini_intersection = (
    ...
    report: dict[str, object] = {
        "schema_version": "phase5a-local-pilot-v3",
        ...
        "v3_valid_results": len(successful_ids),
        "v3_failure_count": len(pending_ids),
        ...
        "technical_reliability_v1_v2_v3": {
            "v1_valid_results": v1_status["successful_at_end"],
            "v1_valid_rate": f"{v1_status['successful_at_end']}/120",
            "v2_valid_results": v2_status["successful_at_end"],
            "v2_valid_rate": f"{v2_status['successful_at_end']}/120",
            "v3_valid_results": len(successful_ids),
            "v3_valid_rate": f"{len(successful_ids)}/120",
            "v1_transport": "prompt/full evidence contract",
            "v2_transport": "native schema/full evidence contract",
            "v3_transport": "native schema/classification-only contract",
            "comparison_scope": (
                "validity, parse/schema failure, truncation, latency, generated tokens; "
                "not score improvement"
            ),
        },
        "session_id": f"phase5a-local-v3-{datetime.now(UTC):%Y%m%d-%H%M%S}",
        ...
        "pending_at_start": len(manifest.example_ids) - len(successful_at_start),
        ...
        "total_pilot_runtime_seconds": round(runtime_seconds, 2),
        "local_llm_latency": local_summary["latency"],
        "local_llm_throughput": _operational_summary(records, runtime_seconds),
        "total_runtime": round(runtime_seconds, 2),
        "phase5b_local_recommendation": "NOT_AUTHORIZED_PRIMARY_INCOMPLETE",
        "phase5b_local_status": "NOT_STARTED",
        "phase5b_full_run_started": False,
        "next_action": (
            "Review the completed classification-first local judge against HHEM before "
            "authorizing a full 2,675-example local benchmark."
            if primary_complete
            else "Resolve the exact V3 local technical failures; do not start Phase 5B-L."
        ),
    }
    if primary_complete:
        report["phase5b_local_recommendation"] = _local_recommendation(
            local_summary, hhem_summary, comparisons["local_vs_hhem"]
        )
...
def _run_consistency(args: argparse.Namespace) -> dict[str, object]:
    ...
    if report.get("experiment_id") != PILOT_ID or report.get("pilot_completed") is not True:
        raise RuntimeError("V3_CONSISTENCY_PRIMARY_INCOMPLETE")
    ...
    for repeat in (1, 2):
        repeat_state = args.report.with_name(
            f"phase5a-local-v3-consistency-repeat-{repeat}-state.json"
        )
        repeat_state.unlink(missing_ok=True)

        def evaluate_local(
            context: str, response: str, example_id: str, repeat_index: int = repeat
        ) -> JudgeEvaluationTrace:
            trace, content = evaluator.evaluate_with_trace_and_content(
                context, response, example_id
            )
            if trace.prediction is None:
                diagnostics.append(trace, content, attempt=repeat_index)
            continuity.observe(trace.model_version)
            return trace
    ...
    report["phase5b_local_recommendation"] = _local_recommendation(
        report["local_llm_results"],
        report["hhem_pilot_results"],
        report["local_llm_vs_hhem"],
        summary,
    )
    report["local_llm_throughput"]["consistency_requests"] = budget.requests_used
    report["total_runtime"] = report["total_pilot_runtime_seconds"]
    report["next_action"] = (
        "Review the completed classification-first local judge against HHEM before "
        "authorizing a full 2,675-example local benchmark."
    )
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the isolated Phase 5A-L v3 local Ollama pilot."
    )
    ...
    try:
        if args.preflight:
            ...
            print(
                json.dumps(
                    {
                        "V3_PREFLIGHT": payload["LOCAL_PREFLIGHT"],
        ...
        result = _run_consistency(args) if args.consistency else _run_pilot(args)
        print(
            json.dumps(
                {
                    "STATUS": result["status"],
                    "V3_VALID_RESULTS": result["v3_valid_results"],
